...
class HuskyLensSensorReader:
    def __init__(self, port="/dev/ttyACM2", baudrate=9600, timeout=0.2, logger=None):
        ...
        if serial is None:
            self._log_error("pyserial is not installed; huskylens serial is unavailable")
            return
    ...
    def _open_serial(self, port):
        try:
            ...
            time.sleep(0.2)
            self._last_open_error = None
            self._log_info(f"HuskyLens serial connected: {port}")
            return True
        except Exception as exc:
            self.ser = None
            msg = str(exc)
            if msg != self._last_open_error:
                self._log_error(f"HuskyLens serial error on {port}: {exc}")
    ...
    def _log_info(self, msg):
        if self._logger:
            self._logger.info(msg, event="SERIAL")
        else:
            print(msg)

    def _log_error(self, msg):
        if self._logger:
            self._logger.error(msg, event="SERIAL")
        else:
            print(msg)
