...
def test_huskylens_strategy_valid_error_bias():
    fsm = LineFollowerFSM(
    ...
    _set_context(
        ...
        huskylens_frame={"connected": 1, "algorithm_set": 1, "valid": 1, "error": -15},
    )
...
def test_huskylens_strategy_deadband_goes_forward():
    fsm = LineFollowerFSM(
        tracking_strategy="huskylens",
        huskylens_control_gain=1.0,
        huskylens_deadband=2.0,
        base_speed=8,
        turn_speed_left=6,
        turn_speed_right=7,
    )
    ...
    _set_context(
        fsm,
        line_sensor_frame=line_frame,
        huskylens_frame={"connected": 1, "algorithm_set": 1, "valid": 1, "error": 1},
    )
    direction, speed = fsm.update(line_frame, now=0.0)
    assert direction == "Forward"
    assert speed == 8
...
def test_huskylens_strategy_out_of_range_fallback_line_sensor_when_not_strict():
    fsm = LineFollowerFSM(
        tracking_strategy="huskylens",
        tracking_strict_mode=False,
        huskylens_max_abs_error=20,
        base_speed=8,
        turn_speed_left=6,
        turn_speed_right=7,
    )
    ...
    _set_context(
        fsm,
        line_sensor_frame=line_frame,
        huskylens_frame={"connected": 1, "algorithm_set": 1, "valid": 1, "error": 50},
    )
    direction, speed = fsm.update(line_frame, now=0.0)
...
def test_hybrid_strategy_prefers_huskylens_then_fallback_line_sensor():
    ...
    _set_context(
        fsm,
        line_sensor_frame=line_frame,
        huskylens_frame={"connected": 1, "algorithm_set": 1, "valid": 1, "error": 10},
    )
    direction, speed = fsm.update(line_frame, now=0.0)
    assert direction == "RotateRight"
    assert speed == 7
    ...
    _set_context(
        fsm,
        line_sensor_frame=line_frame,
        huskylens_frame={"connected": 0, "algorithm_set": 0, "valid": 0, "error": 0},
    )
...
def test_strict_mode_stops_when_selected_strategy_invalid():
    ...
    _set_context(
        fsm,
        line_sensor_frame=line_frame,
        huskylens_frame={"connected": 1, "algorithm_set": 1, "valid": 0, "error": 0},
    )
